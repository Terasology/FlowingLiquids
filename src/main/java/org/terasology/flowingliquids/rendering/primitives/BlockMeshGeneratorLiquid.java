...
public class BlockMeshGeneratorLiquid implements BlockMeshGenerator {
    ...
    public void generateChunkMesh(ChunkView view, ChunkMesh chunkMesh, int x, int y, int z) {
        ...
        boolean suppressed = view.getBlock(pos.x, pos.y + 1, pos.z) == block; // Render it as full even though it 
        // actually isn't.
        boolean full = suppressed || isFull(renderHeight);

        BlockAppearance appearance = block.getAppearance(null); //TODO: collect information the block wants, or avoid
        // this entirely.
...
//                Vector4f colorOffset = block.calcColorOffsetFor(BlockPart.fromSide(side), biome);
    ...
    private float[] getRenderHeight(ChunkView view, Vector3i pos) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                if (view.getBlock(pos.x + x, pos.y, pos.z + z) == block && view.getBlock(pos.x + x, pos.y + 1,
                        pos.z + z) != block) {
    ...
    private BlockMeshPart lowerPart(Side side, BlockMeshPart basePart, float[] heights, boolean suppressed,
                                    boolean matches) {
    ...
    private boolean isSideVisibleForBlockTypes(Block blockToCheck, boolean adjacentSuppressed, Block currentBlock,
                                               boolean full, boolean suppressed, Side side) {
